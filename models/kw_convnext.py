...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6,
        ...
        super().__init__()
        self.dwconv = warehouse_manager.reserve(dim, dim, kernel_size=7, padding=3, groups=dim,
                                                warehouse_name='stage{}_layer{}_dwconv{}'.format(stage_idx, layer_idx, 0))  # depthwise conv
        self.norm = LayerNorm(dim, eps=1e-6)
        self.pwconv1 = warehouse_manager.reserve(dim, 4 * dim, kernel_size=1, padding=0,
                                                 warehouse_name='stage{}_layer{}_pwconv{}'.format(stage_idx, layer_idx, 1))  # pointwise/1x1 convs, implemented with linear layers
        self.act = nn.GELU()
        self.pwconv2 = warehouse_manager.reserve(4 * dim, dim, kernel_size=1, padding=0,
...
class KW_ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 sharing_range=('layer', 'pwconv'), nonlocal_basis_ratio=1, **kwargs
                 ):
        super().__init__()
        ...
        stem = nn.Sequential(
            self.warehouse_manager.reserve(in_chans, dims[0], kernel_size=4, stride=4,
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                ...
                self.warehouse_manager.reserve(dims[i], dims[i + 1], kernel_size=2, stride=2,
...
@register_model
def kw_convnext_tiny(pretrained=False, in_22k=False, **kwargs):
    model = KW_ConvNeXt(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], **kwargs)
    return model